...
describe("XCN[Annotation-Author] Converter", () => {
  describe("convertXCNToAnnotationAuthor", () => {
    ...
    test("converts basic XCN with ID and name", () => {
      ...
      expect(result?.annotation.authorReference?.reference).toBe(result?.practitionerId);
    ...
    test("generates unique IDs for multiple conversions", () => {
      ...
      expect(result1?.practitionerId).toBeDefined();
      expect(result2?.practitionerId).toBeDefined();
    ...
    test("authorReference matches practitioner ID", () => {
      ...
      expect(result).toBeDefined();
      expect(result?.annotation.authorReference?.reference).toBe(result?.practitioner.id);
      expect(result?.annotation.authorReference?.reference).toBe(result?.practitionerId);
    });
