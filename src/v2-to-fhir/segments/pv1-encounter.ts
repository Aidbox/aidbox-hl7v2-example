...
// Maps RAW HL7v2 patient class codes (PV1-2) to location status
// Used by sync functions that don't do ConceptMap resolution
...
// Used by async functions after ConceptMap resolution
...
/**
 * Result type for PV1 conversion with mapping support.
 * Returns either an Encounter or a mapping error for the Patient Class field.
 */
export type PV1ConversionResult =
  | { encounter: Encounter; error?: never }
  | { encounter?: never; error: MappingError };

/**
 * Convert PV1 segment to FHIR Encounter with mapping error support.
 *
 * Instead of preserving unknown Patient Class codes in v2-0004 system,
 * this version returns a mapping error that can be used to create a Task
 * for manual resolution.
 *
 * Use this function when you want to collect mapping errors for Task creation.
 * The original convertPV1ToEncounter() function is kept for backward compatibility.
 */
export function convertPV1WithMappingSupport(pv1: PV1): PV1ConversionResult {
  // Check discharge datetime to determine status
  const hasDischargeDateTime = !!(pv1.$45_discharge?.[0]);

  // Extract and normalize patient class
  let classCode = "U";
  if (pv1.$2_class) {
    if (typeof pv1.$2_class === "string") {
      classCode = pv1.$2_class.toUpperCase();
  ...
  // Try to map patient class
  const classResult = mapPatientClassToFHIRWithResult(classCode, hasDischargeDateTime);

  if (classResult.error) {
    return { error: classResult.error };
  }

  // Build the Encounter with the mapped class and status
  const encounter: Encounter = {
    resourceType: "Encounter",
  ...
  // =========================================================================
  // Identifiers (same as original function)
  // =========================================================================
...
/**
 * Convert PV1 segment to FHIR Encounter with mapping error support (async version).
 *
 * This version checks ConceptMap for sender-specific patient class mappings.
 ...
 * @param sender - The sender context for ConceptMap lookup
 */
export async function convertPV1WithMappingSupportAsync(
  pv1: PV1,
  sender: SenderContext,
  ...
  // Resolve patient class with ConceptMap lookup
  ...
  // Store resolved FHIR class code for location status determination
  const resolvedFhirClassCode = classResult.class.code;
  ...
  // PV1-3: Assigned Patient Location -> location[1]
  // Status depends on the RESOLVED FHIR class code: if PRENC (pre-admission) -> planned, else active
  // We use the resolved class code rather than the raw PV1-2 code to correctly handle
  // ConceptMap-resolved custom patient class codes (e.g., "1" -> PRENC)
  const assignedLocationStatus: EncounterLocation["status"] =
