 ...
 * Accounts UI Module
 *
 * Displays the Accounts page for BAR message generation.
 */
...
interface AccountListItem {
  id: string;
  status: string;
  subject?: Array<{ reference: string }>;
  extension?: Array<{
    url: string;
    valueCode?: string;
    valueString?: string;
...
export async function handleAccountsPage(req: Request): Promise<Response> {
  ...
  const [
    ...
    getAccounts(statusFilter, requestedPage),
    getPatients(),
    getConditions(),
    getProcedures(),
    getPendingAccountCount(),
    getErrorAccountCount(),
    getNavData(),
  ]);

  const pagination = createPagination(requestedPage, accountsResult.total);
...
export async function createAccount(req: Request): Promise<Response> {
  ...
  const conditionRefs = formData.getAll("conditions") as string[];
  const procedureRefs = formData.getAll("procedures") as string[];
  const servicePeriodStart = formData.get("servicePeriodStart") as string;
  const servicePeriodEnd = formData.get("servicePeriodEnd") as string;
  ...
  for (const ref of conditionRefs) {
    extensions.push({
      url: "http://example.org/account-diagnosis",
      extension: [{ url: "condition", valueReference: { reference: ref } }],
    });
  }

  for (const ref of procedureRefs) {
    extensions.push({
      url: "http://example.org/account-procedure",
      extension: [{ url: "procedure", valueReference: { reference: ref } }],
    });
  }
  ...
  if (subject) {
    newAccount.subject = [{ reference: subject }];
  }

  if (accountType) {
    newAccount.type = {
      coding: [{ system: "http://terminology.hl7.org/CodeSystem/v3-ActCode", code: accountType }],
    };
  }

  if (servicePeriodStart || servicePeriodEnd) {
    newAccount.servicePeriod = {
      ...(servicePeriodStart && { start: servicePeriodStart }),
      ...(servicePeriodEnd && { end: servicePeriodEnd }),
    };
  }

  await aidboxFetch("/fhir/Account", {
    method: "POST",
...
function getProcessingStatus(account: AccountListItem): string {
...
function getErrorReason(account: AccountListItem): string | undefined {
  const ext = account.extension?.find(
    (e) => e.url === "http://example.org/account-processing-error-reason",
  );
  return ext?.valueString;
}

function getAccountRetryCount(account: AccountListItem): number {
  const ext = account.extension?.find(
    (e) => e.url === "http://example.org/account-processing-retry-count",
  );
...
const getConditions = () => getResources<Condition>("Condition", "_sort=-_lastUpdated");
const getProcedures = () => getResources<Procedure>("Procedure", "_sort=-_lastUpdated");

async function getAccounts(
  processingStatus?: string,
  page = 1,
): Promise<{ accounts: AccountListItem[]; total: number }> {
...
async function getPendingAccountCount(): Promise<number> {
  const bundle = await aidboxFetch<Bundle<AccountListItem>>(
    "/fhir/Account?processing-status=pending&_count=0",
  );
  return bundle.total ?? 0;
}

async function getErrorAccountCount(): Promise<number> {
  const bundle = await aidboxFetch<Bundle<AccountListItem>>(
    "/fhir/Account?processing-status=error&_count=0",
  );
  return bundle.total ?? 0;
}
...
function renderAccountsPage(
  ...
  const accountItems = accounts
    .map((acct) => {
      const processingStatus = getProcessingStatus(acct);
      const errorReason = getErrorReason(acct);
      const retryCount = getAccountRetryCount(acct);
      const statusClass =
        ...
        <details class="group">
          <summary class="cursor-pointer list-none p-4 flex items-center justify-between hover:bg-gray-50 rounded-lg">
            <div class="flex items-center gap-3">
              ...
              <span class="font-mono text-sm font-medium">${acct.id}</span>
            ...
            <div class="flex items-center gap-4 text-sm text-gray-500">
              ${acct.subject?.[0]?.reference ? `<span><span class="text-gray-400">Subject:</span> ${acct.subject[0].reference}</span>` : ""}
              <span><span class="text-gray-400">Status:</span> ${acct.status}</span>
            </div>
          </summary>
          <div class="px-4 pb-4">
            <pre class="p-3 bg-gray-50 rounded font-mono text-xs overflow-x-auto whitespace-pre">${JSON.stringify(acct, null, 2)}</pre>
          </div>
        </details>
      </li>`;
    })
  ...
  const content = `
    <div class="flex items-center justify-between mb-6">
      ...
      <div class="flex gap-2">
        ...
        <button onclick="document.getElementById('add-account-form').classList.toggle('hidden')" class="px-4 py-2 bg-green-600 text-white rounded-lg font-medium hover:bg-green-700 flex items-center gap-2">
    ...
    <div class="mb-4 flex gap-2">
      <a href="/accounts" class="px-3 py-1.5 rounded-lg text-sm font-medium ${!statusFilter ? "bg-gray-800 text-white" : "bg-gray-200 text-gray-700 hover:bg-gray-300"}">
        All
      </a>
      ${processingStatuses
        .map(
          (s) => `
        <a href="/accounts?processing-status=${s}" class="px-3 py-1.5 rounded-lg text-sm font-medium ${statusFilter === s ? "bg-gray-800 text-white" : "bg-gray-200 text-gray-700 hover:bg-gray-300"}">
    ...
    <div id="add-account-form" class="hidden mb-6 bg-white rounded-lg shadow p-6">
      <h2 class="text-lg font-semibold text-gray-800 mb-4">Add New Account</h2>
      <form method="POST" action="/accounts" class="space-y-4">
        <div>
        ...
        <div class="grid grid-cols-3 gap-4">
          ...
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">Service Period Start</label>
            <input type="date" name="servicePeriodStart" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
          </div>
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">Service Period End</label>
            <input type="date" name="servicePeriodEnd" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
          </div>
        </div>
        <div class="grid grid-cols-2 gap-4">
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">Conditions (Diagnoses)</label>
            <select name="conditions" id="conditions-select" multiple class="w-full h-32 px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
              ${conditions
                .map((c) => {
                  const patientRef = c.subject?.reference || "";
                  const code = c.code?.coding?.[0];
                  const display = code?.display || code?.code || c.id;
                  return `<option value="Condition/${c.id}" data-patient="${patientRef}">${display} (${c.id})</option>`;
                })
                .join("")}
            </select>
          </div>
        ...
        <div class="flex gap-2">
          ...
          <button type="button" onclick="document.getElementById('add-account-form').classList.add('hidden')" class="px-4 py-2 bg-gray-200 text-gray-700 rounded-lg font-medium hover:bg-gray-300">
            Cancel
          </button>
        </div>
      </form>
    ...
    <ul class="space-y-2">
      ${accountItems || '<li class="bg-white rounded-lg shadow p-8 text-center text-gray-500">No accounts found</li>'}
    </ul>
    <div class="mt-4 flex items-center justify-between">
      <p class="text-sm text-gray-500">Total: ${pagination.total} accounts</p>
