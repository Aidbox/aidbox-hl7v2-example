...
**Path A: Mapping Tasks Queue**
1. User resolves task by selecting LOINC
2. System writes ConceptMap entry and marks Task `completed`
3. Find affected messages with `status=mapping_error` and matching task
4. Remove resolved entry from `unmappedCodes[]`
5. If empty → set `status=received` for reprocessing

**Path B: ConceptMap Table**
1. User creates ConceptMap entry directly
2. System finds matching Task(s) and marks them `completed`
3. Find affected messages with `status=mapping_error` and matching task
4. Remove resolved entry from `unmappedCodes[]`
5. If empty → set `status=received` for reprocessing
...
- Deterministic Task ID: `{senderId}-{hash(localSystem)}-{hash(localCode)}` via PUT (upsert)
- Use `If-Match` with ETag when updating `IncomingHL7v2Message.unmappedCodes[]`
- Add `sendingApplication` and `sendingFacility` at MLLP storage time, even on parse failure
