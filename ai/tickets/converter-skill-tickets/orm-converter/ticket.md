...
- [x] Create `test/unit/v2-to-fhir/messages/orm-o01.test.ts` following the pattern in `oru-r01.test.ts` (inline HL7v2 message strings, `makeTestContext()`)
- [x] Test: ORM with single OBR order produces ServiceRequest with correct resourceType, intent="order", code, identifiers [AC-3]
- [x] Test: ORM with single RXO order produces MedicationRequest with intent="original-order", medication code [AC-4]
- [x] Test: ORM with multiple OBR orders produces multiple ServiceRequests with distinct IDs [AC-9]
- [x] Test: ORM with multiple RXO orders produces multiple MedicationRequests [AC-9]
- [x] Test: DG1 segments produce Conditions linked via reasonReference [AC-5]
- [x] Test: NTE segments produce ServiceRequest.note entries [AC-6]
- [x] Test: OBX segments produce Observations linked via supportingInfo [AC-7]
- [x] Test: IN1 segments produce Coverage resources [AC-8]
- [x] Test: OBX in ORM context does NOT trigger LOINC resolution [AC-21]
- [x] Test: Missing OBX-11 defaults to status=registered [RELAX-5]
- [x] Test: ServiceRequest.status from ORC-5 standard value "SC" -> "active" [AC-15]
- [x] Test: ServiceRequest.status from ORC-1 "NW" when ORC-5 empty -> "active" [AC-15]
- [x] Test: Status = "unknown" when both ORC-1 and ORC-5 empty [AC-15, RELAX-1]
- [x] Test: Missing PID rejects message (status="error") [AC-18]
- [x] Test: Missing ORC-2 and OBR-2 rejects order group [AC-20]
- [x] Test: ORM without PV1 processes normally (status=processed) [AC-12]
- [x] Test: ORM with empty PV1 processes normally (treated as absent) [EC-1]
- [x] Test: ORM with valid PV1-19 creates Encounter [AC-13]
- [x] Test: ORM with PV1 but no PV1-19 skips Encounter (status=processed) [FALL-6]
- [x] Test: Deterministic ServiceRequest ID from ORC-2 [AC-10, FALL-1]
- [x] Test: Deterministic Condition ID from order + position [AC-10, FALL-3]
- [x] Test: Deterministic Observation ID from order + position [AC-10, FALL-4]
- [x] Test: OBR-11 "G" overrides intent to "reflex-order" [REQ-OBR-1]
- [x] Test: ORC-12 maps to ServiceRequest.requester [REQ-ORC-1]
- [x] Test: OBR-16 used as requester fallback when ORC-12 empty [EC-7]
- [x] Test: Mixed order types in one message: OBR + RXO -> 1 ServiceRequest + 1 MedicationRequest [EC-2]
- [x] Test: NTE in RXO order maps to MedicationRequest.note [REQ-RXO-LINKAGE-1]
- [x] Test: DG1 in RXO order maps to MedicationRequest.reasonReference [REQ-RXO-LINKAGE-1]
- [x] Test: OBX in RXO order maps to MedicationRequest.supportingInformation [REQ-RXO-LINKAGE-1]
- [x] Run `bun test:all` and `bun run typecheck` - must pass before next task
