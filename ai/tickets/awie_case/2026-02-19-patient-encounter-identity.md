...
- **MatchRule** `{ assigner?, type?, any? }` — matches the first CX where the assigner matches any of CX.4.1, CX.9.1 (CWE.1), or CX.10.1 (CWE.1) in that order, and/or CX.5 equals the type. A special variant `{ any: true }` matches the first CX with a non-empty CX.1 and a derivable assigner prefix (type-only priority chain); bare CX entries with no authority are skipped. At least one of `assigner`, `type`, or `any` must be specified.
...
The resulting Patient.id format is `{sanitize(assigner)}-{sanitize(value)}` using the same sanitization pattern already applied to Encounter.id in `id-generation.ts`.
...
|----------|-------------------|--------|-----------|
| Identifier selection strategy | Priority-list (assigner + type rules, ordered) vs type-only vs assigner-only | Priority-list with both assigner and type matchers | Real data shows UNIPAT appears in different fields across senders; assigner rules target specific namespaces, type rules act as spec-driven fallbacks. Fixed type-only or assigner-only cannot express "UNIPAT first, then any PE, then ST01, then any MR". |
...
| ID assigner source | Rule's stated assigner vs matched identifier's own assigner | Matched identifier's own assigner | The rule selects WHICH identifier; the identifier provides the system context. Rule `{ type: "MR" }` matching `ST01W/MR` → ID is `st01w-645541`. Using the rule's assigner would be wrong when the rule has no assigner (type-only rule). |
...
| MatchRule assigner matching components | CX.4.1 only vs CX.4.1 → CX.9.1 → CX.10.1 (all three HL7 authority components) vs extractHDAuthority semantics (CX.4.2 preferred) | Check CX.4.1 first, then CX.9.1 (CWE.1), then CX.10.1 (CWE.1); first non-empty that equals rule.assigner wins. ID prefix split by match type: assigner-rule match uses the component that matched (CX.4 hierarchy / CX.9.1 / CX.10.1); type-only match uses priority chain CX.4.1 → CX.4.2 → CX.9.1 → CX.10.1 → raw CX.4 string | HL7 v2.8.2 allows authority in CX.4, CX.9, or CX.10 — all three must be checked for completeness. Config entries remain human namespace strings ("UNIPAT", "ST01") so CX.4.1/CX.9.1/CX.10.1 (the short identifier subcomponents, not OIDs) are the correct matching targets. CX.4 is preferred for type-only match prefix because it is the most commonly populated authority component. This deliberately deviates from extractHDAuthority (which prefers CX.4.2 for FHIR system URI selection) — selectPatientId picks the short namespace prefix for a resource ID, not a URI. |
...
| `src/v2-to-fhir/config.ts` | Modify | Restructure `Hl7v2ToFhirConfig` to `{ identitySystem?: { patient?: { rules: IdentifierPriorityRule[] } }, messages?: Record<string, MessageTypeConfig> }`, extend `MessageTypeConfig.preprocess` with PID fields, update `validatePreprocessorIds` to walk `config.messages` instead of top-level config object; add `validateIdentitySystemRules()` called from `hl7v2ToFhirConfig()` before caching — validates: (1) `identitySystem.patient.rules` is a non-empty array, (2) each MatchRule has at least one of `assigner`/`type`/`any`; add runtime guard before caching: `if (!Array.isArray(config.identitySystem?.patient?.rules)) throw new Error('...')`; import `PatientIdResolver` from `./id-generation` if needed (type is defined and exported ONLY from `id-generation.ts`, not re-exported from `config.ts`) |
...
/** Match rule: select first CX where assigner and/or type match. At least one of assigner, type, or any must be specified. */
export type MatchRule = {
  /**
   * Matches the assigning authority of a CX entry by checking CX.4.1 (HD Namespace ID) first,
   ...
   *
   * ID prefix when this assigner matches:
   ...
   */
  assigner?: string; // matched against CX.4.1, then CX.9.1, then CX.10.1 (case-sensitive)
  type?: string;      // match CX.5 exactly
  /**
   * When true, matches the first CX with a non-empty CX.1 regardless of assigner or type.
   * ID prefix uses the type-only priority chain: CX.4.1 → CX.4.2 → CX.9.1 → CX.10.1 → raw CX.4 string.
   ...
   */
  any?: true;
  // Constraint: at least one of assigner, type, or any must be present (validated at config load time)
};
...
export type MpiLookupRule = {
  mpiLookup: {
    ...
    target: {
      ...
      assigner: string; // HL7v2 assigner string for the resulting Patient.id
      type?: string;
    };
 ...
 *    a. MatchRule { assigner?, type?, any? }:
 *       - If rule.any is true:
 *         Match the first CX with non-empty CX.1 regardless of assigner or type.
 *         Derive prefix using the type-only priority chain: CX.4.1 → CX.4.2 → CX.9.1 → CX.10.1 → raw CX.4 string.
 ...
 *       - assigner check (if rule.assigner is set): check CX.4.1 → CX.9.1 (CWE.1) → CX.10.1 (CWE.1)
 *         in that order. The first non-empty component that equals rule.assigner wins.
 ...
 *       - On match — derive the ID assigner prefix (split by match type):
 *
 *         ASSIGNER-RULE MATCH (rule.assigner is set, matched a specific component):
 ...
 *
 *         TYPE-ONLY MATCH (rule has type but no assigner):
 *           priority chain: CX.4.1 → CX.4.2 → CX.9.1 → CX.10.1 → raw CX.4 string
 *
 *           return { id: `${sanitize(assignerPrefix)}-${sanitize(CX.1)}` }
 *    b. MpiLookupRule:
 ...
 *       iii. status='found' → return { id: `${sanitize(target.assigner)}-${sanitize(result.value)}` }
 ...
 *
 * Assigner component semantics (deliberate deviation from extractHDAuthority):
 ...
 *   - ID FORMATION: assigner prefix source depends on which component matched (assigner rules)
 *     or uses a priority chain CX.4.1 → CX.4.2 → CX.9.1 → CX.10.1 → raw CX.4 string (type-only rules).
...
function validateIdentitySystemRules(config: Hl7v2ToFhirConfig): void {
  ...
  for (let i = 0; i < rules.length; i++) {
    ...
    if ('mpiLookup' in rule) {
      ...
      // MatchRule validation: at least one of assigner, type, or any must be set
      if (!rule.assigner && !rule.type && !rule.any) {
        throw new Error(
          `Invalid identitySystem.patient.rules[${i}]: MatchRule must specify at least one of: ` +
          `"assigner" (matched against CX.4.1, CX.9.1, or CX.10.1), "type" (matches CX.5), or "any" (true).`
        );
      }
    }
  }
...
| CX with only CX.4.2 (Universal ID / OID), no CX.4.1 (namespace), and no CX.9/CX.10 | MatchRule `{ assigner: "..." }` will not match — matching checks CX.4.1, CX.9.1, CX.10.1, all of which are empty. The CX is still eligible for type-only rules. For ID formation under a type-only rule, the priority chain (CX.4.1 → CX.4.2 → CX.9.1 → CX.10.1 → raw CX.4 string) falls through to CX.4.2 as the first non-empty value. Config entries must use the .1 identifier subcomponents, not OIDs. If a sender provides identifiers only via OID in CX.4.2, use a type-only rule and accept the OID as the authority prefix in the Patient.id. |
...
| MatchRule specifies neither assigner, type, nor any | Validated at config load time via `validateIdentitySystemRules()`: throws `Error("MatchRule must specify at least one of: assigner, type, or any")`. |
...
| CX with only CX.9 or CX.10 populated (no CX.4) | `inject-authority-from-msh` checks all of CX.4/9/10 — if CX.9 or CX.10 is non-empty, the CX is treated as already having an authority and is not modified. `MatchRule.assigner` matching checks CX.4.1 (empty), then CX.9.1, then CX.10.1 — so an assigner rule like `{ assigner: "STATEX" }` WILL match if CX.9.1 or CX.10.1 equals "STATEX". The ID prefix is then derived from CX.9.1 or CX.10.1 respectively (not from the empty CX.4). This is correct: CX.9 (jurisdiction) and CX.10 (department) are valid HL7 authority sources and must be first-class matching targets. |
| CX with authority in CX.9.1 matched by assigner rule | MatchRule `{ assigner: "STATEX" }` matches CX where CX.4.1 is empty but CX.9.1 = "STATEX". Prefix = "STATEX" (from CX.9.1). Returns `{ id: "statex-{sanitize(CX.1)}" }`. |
| CX with authority in CX.10.1 matched by assigner rule | MatchRule `{ assigner: "DEPT01" }` matches CX where CX.4.1 and CX.9.1 are empty but CX.10.1 = "DEPT01". Prefix = "DEPT01" (from CX.10.1). Returns `{ id: "dept01-{sanitize(CX.1)}" }`. |
| Type-only rule with CX that has CX.9.1 populated (CX.4 empty) | Rule `{ type: "MR" }` matches CX with CX.5="MR", CX.4.1 and CX.4.2 empty, CX.9.1="STATEX". Type-only prefix chain: CX.4.1 empty → CX.4.2 empty → CX.9.1="STATEX" wins. Returns `{ id: "statex-{sanitize(CX.1)}" }`. |

## Test Cases

| Test Case | Type | Description |
|-----------|------|-------------|
| ASTRA message: UNIPAT in PID-2, ST01W/MR + ST01/PI in PID-3 — after move-pid2-into-pid3, rule `{assigner: "UNIPAT"}` matches via CX.4.1 | Unit | CX has CX.4.1="UNIPAT"; matching checks CX.4.1 first, finds match; prefix = "UNIPAT" (CX.4 hierarchy). selectPatientId returns `unipat-11195429` |
| MEDTEX message: UNIPAT directly in PID-3 — rule `{assigner: "UNIPAT"}` matches via CX.4.1 without preprocessing | Unit | CX has CX.4.1="UNIPAT"; CX.4.1 check matches immediately. selectPatientId returns `unipat-11216032` |
...
| Xpan lab: `&&ISO`/MR, `&&ISO`/PI, `&&ISO`/AN — rule `{type: "MR"}` matches at position 4, type-only prefix falls to raw CX.4 string | Unit | CX has CX.5="MR", CX.4.1 empty, CX.4.2 empty, CX.9.1 empty, CX.10.1 empty. Type-only prefix chain exhausts CX.4.1/CX.4.2/CX.9.1/CX.10.1, falls back to raw CX.4 string `&&ISO` → sanitized to `--iso`. selectPatientId returns `--iso-m000000721`. |
| Assigner rule matches via CX.9.1 — CX.4.1 is empty but CX.9.1 equals rule.assigner | Unit | rule `{assigner: "STATEX"}`, CX has CX.4.1="" and CX.9.1="STATEX". Matching: CX.4.1 empty → check CX.9.1 = "STATEX" — matches. Prefix = CX.9.1 = "STATEX". Returns `statex-{sanitize(CX.1)}`. |
| Assigner rule matches via CX.10.1 — CX.4.1 and CX.9.1 are empty but CX.10.1 equals rule.assigner | Unit | rule `{assigner: "DEPT01"}`, CX has CX.4.1="" and CX.9.1="" and CX.10.1="DEPT01". Matching: CX.4.1 empty → CX.9.1 empty → CX.10.1 = "DEPT01" — matches. Prefix = CX.10.1 = "DEPT01". Returns `dept01-{sanitize(CX.1)}`. |
| Assigner rule does NOT match when assigner string is only in CX.4.2 (Universal ID), not in CX.4.1/CX.9.1/CX.10.1 | Unit | rule `{assigner: "UNIPAT"}` does not match CX with CX.4.2="UNIPAT", CX.4.1="" (CX.9 and CX.10 empty). CX.4.1, CX.9.1, CX.10.1 all empty — no match. CX eligible for type-only rules only. |
| Type-only rule with CX that has both CX.9.1 and CX.4.1 — prefix comes from CX.4.1 (preferred over CX.9.1) | Unit | rule `{type: "MR"}`, CX has CX.5="MR", CX.4.1="ST01", CX.9.1="STATEX". Type-only prefix chain: CX.4.1="ST01" wins first (CX.4 is preferred as direct assigning authority). Returns `st01-{sanitize(CX.1)}`. |
| Type-only rule with CX that has CX.4.1 empty and CX.9.1 populated — prefix comes from CX.9.1 | Unit | rule `{type: "MR"}`, CX has CX.5="MR", CX.4.1="", CX.4.2="", CX.9.1="STATEX". Type-only prefix chain: CX.4.1 empty → CX.4.2 empty → CX.9.1="STATEX" wins. Returns `statex-{sanitize(CX.1)}`. |
| Type-only rule with CX that has CX.10.1 only (CX.4.1/CX.4.2/CX.9.1 empty) | Unit | rule `{type: "AN"}`, CX has CX.5="AN", CX.4.1="", CX.4.2="", CX.9.1="", CX.10.1="DEPT01". Type-only prefix chain: CX.4.1→CX.4.2→CX.9.1 all empty → CX.10.1="DEPT01" wins. Returns `dept01-{sanitize(CX.1)}`. |
| MatchRule ID formation: CX with empty CX.4.1/CX.9.1/CX.10.1 and non-empty CX.4.2 matched by type-only rule produces CX.4.2-based prefix | Unit | rule `{type: "MR"}` matching CX with CX.4.1="", CX.4.2="urn:oid:2.16.840.1.113883.1.111", CX.9.1="", CX.10.1="", CX.1="12345". Type-only chain: CX.4.1 empty → CX.4.2="urn:oid:..." wins. Returns `urn:oid:2-16-840-1-113883-1-111-12345`. |
...
| MPI rule (pix strategy): no source identifier in pool — skip | Unit | **Deferred to MPI implementation ticket** — source selection logic not in scope; StubMpiClient returns not-found for all pix calls |
| MpiLookupRule (pix strategy): source identifier selected via CX.9.1 — MPI finds result | Unit | **Deferred to MPI implementation ticket** — source selection logic not in scope |
...
| `{ any: true }` matches first CX with derivable prefix | Unit | Pool has CX1 (CX.4.1="BMH", CX.1="123") and CX2 (CX.4.1="ST01", CX.1="456"). `{ any: true }` matches CX1 first; type-only prefix chain: CX.4.1="BMH" wins. Returns `bmh-123`. |
...
| MatchRule with neither assigner, type, nor any | Unit | Config load throws validation error via `validateIdentitySystemRules()` |
...
| **Migrated: config.test.ts** — valid config returns typed object with new shape | Unit | `configWithMshFallback` fixture updated to `{ identitySystem: { patient: { rules: [{assigner: "UNIPAT"}] } }, messages: { "ORU-R01": {...}, "ADT-A01": {...} } }`; `config["ORU-R01"]` access changes to `config.messages["ORU-R01"]`; matching semantics in rule description updated to reflect CX.4.1 → CX.9.1 → CX.10.1 check order |
...
2. **BLOCKER 2 ({ any: true }):** FIXED. All six required locations updated:
   ...
   - validateIdentitySystemRules accepts `{ any: true }` via `!rule.assigner && !rule.type && !rule.any` (line 362)
...
Add `any?: true` to `MatchRule`. A rule of `{ any: true }` matches the first CX with a non-empty CX.1 regardless of assigner or type. ID prefix derivation uses the type-only priority chain (CX.4.1 → CX.4.2 → CX.9.1 → CX.10.1 → raw CX.4 string). If no authority prefix can be derived (all components empty — bare CX), `{ any: true }` skips that CX and moves to the next. Update `validateIdentitySystemRules` to accept `{ any: true }` as a valid rule (no assigner or type required for this variant). Rules array must still be non-empty (empty array = always-failing config = startup error).
...
- [x] Implement `selectPatientId(identifiers, rules, mpiClient)` following the algorithm in design Technical Details:
  ...
  - For each rule in order: handle `{ any: true }` (match first CX with derivable prefix, skip bare CX), assigner matching (CX.4.1 → CX.9.1 → CX.10.1), type matching (CX.5), combined assigner+type
  - Authority-rule prefix: matched via CX.4.1 → CX.4 hierarchy, CX.9.1 → CX.9.1, CX.10.1 → CX.10.1
  - Type-only / `{ any: true }` prefix chain: CX.4.1 → CX.4.2 → CX.9.1 → CX.10.1 → raw CX.4 string
...
- [x] Write unit tests in `test/unit/v2-to-fhir/select-patient-id.test.ts` covering ALL test cases from design:
  ...
  - Empty CX.1 → skipped
  - MPI pix: found (throws — not implemented), not-found (falls through), unavailable (hard error)
...
- [x] Update `src/v2-to-fhir/config.ts`:
  ...
  - Import `IdentifierPriorityRule` from `./identity-system/patient-id`
  - Add `validateIdentitySystemRules(config)` function (Guards 1-3 from design: array present, non-empty, each MatchRule has assigner/type/any)
  - Call `validateIdentitySystemRules` before `validatePreprocessorIds` in `hl7v2ToFhirConfig()`
  - Update `validatePreprocessorIds` to iterate `config.messages` instead of top-level `Object.entries(config)`
- [x] Update `config/hl7v2-to-fhir.json`: replace flat format with new shape (`identitySystem.patient.rules` + `messages` record). Remove `_DESIGN_PROTOTYPE` and `_DESIGN_PROTOTYPE_new_shape` comment keys
- [x] Update `src/v2-to-fhir/preprocessor.ts`:
  - Change `config[configKey]` → `config.messages?.[configKey]`
...
- [x] Migrate `test/unit/v2-to-fhir/config.test.ts`:
  - All fixture objects: flat `{ "ORU-R01": {...} }` → `{ identitySystem: { patient: { rules: [{ assigner: "UNIPAT" }] } }, messages: { "ORU-R01": {...} } }`
  ...
  - Add test: MatchRule with neither assigner, type, nor any → throws at startup
  - Add test: empty rules array → throws at startup
- [x] Migrate `test/unit/v2-to-fhir/preprocessor.test.ts`:
  - `configWithMshFallback` → `{ identitySystem: { patient: { rules: [{ assigner: "UNIPAT" }] } }, messages: { "ORU-R01": {...}, "ADT-A01": {...} } }`
  - `configWithoutPreprocess` → same structure without `preprocess` keys
  - Inline `config` objects in tests: same migration
- [x] Run `bun test:all` and `bun run typecheck` — must pass before next task (1 pre-existing failure in select-patient-id MPI source test, not introduced by this task)
